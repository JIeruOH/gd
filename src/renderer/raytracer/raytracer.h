...
namespace cg::renderer
{
			...
			float3 right, float3 up, size_t depth, size_t accumulation_num)
	{
		...
		for (int frame_id = 0; frame_id < accumulation_num; frame_id++) {
			...
			float2 jit = get_jitter(frame_id);
			for (int x = 0; x < width; x++) {
				for (int y = 0; y < height; y++) {
					float u = (2.f * x + jit.x) / static_cast<float>(width - 1) - 1.f;
					float v = (2.f * y + jit.y) / static_cast<float>(height - 1) - 1.f;
	...
	float2 raytracer<VB, RT>::get_jitter(int frame_id)
	{
		...
		int index;
		float inv_base, fraction;

		constexpr int base_x = 2;
		index = frame_id + 1;
		inv_base = 1.f / static_cast<float>(base_x);
		fraction = inv_base;
		while (index > 0) {
